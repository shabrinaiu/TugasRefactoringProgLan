...
public class PirateFrame extends javax.swing.JFrame {
    ...
    private void transferMessage(int id, String name){
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    disableButton();
                    client.sendMessage(new Message(id, name));
    ...
    private void checkCondition(int pirateId, Message message){
        conditionTextField.setText("Townie chose " + message.getName()
        + " to defend. " + pirate.setCondition(pirate.winCondition(pirateId, message.getId())));
        if (!pirate.winCondition(pirateId, message.getId())) {
            pirate.subtractHealth(pirate.getHealth());
            healthValue.setText(String.valueOf(pirate.getHealth()));
        } 
